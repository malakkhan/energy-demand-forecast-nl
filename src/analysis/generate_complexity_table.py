...
# Only report CMAT-Full time means once every in-scope horizon has its full
# 3-seed x 10-fold complement — means over a partial (early-fold-heavy)
# subset would be biased low, since fold cost grows with the training window.
def _full_ready():
    if full is None:
        return False
    d = full[full.fold <= 9]
    return all(len(d[d.horizon_days == h]) >= 30 for h in HORIZONS)
...
Prophet, and the na\"ive baselines use CPU only.""" + (
    "" if FULL_READY else
    " CMAT-Full timing pending (ROCV in progress)."
) + r"""}
...
GROUP_SC = (r" &  &  & \multicolumn{3}{c}{\textbf{Raw observations}} & "
            ...
            r"\multicolumn{3}{c}{\textbf{CMAT-Full sequences}} \\")

print(r"""\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
...
for hi, h in enumerate(HORIZONS):
    ...
    for fold_idx, train_end, val_end, test_origin in folds_list:
        ...
        cells = (raw + seq_counts(tab, h, fold_idx) + seq_counts(ntl, h, fold_idx)
                 + (seq_counts(full, h, fold_idx) if full is not None else ["---"] * 3))
